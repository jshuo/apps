...
function BannerExtension (): React.ReactElement | null {
  ...
  return (
    <Banner type='warning'>
      ...
      {/* <ul>
      <li>{t<string>('Hardware wallets provide the best security for your private key since it never leaves the device when signing transactions. ')}</li>
      <li>{t<string>('Connect to SecuX Hardware Wallet via USB to manager your Aleph Zero Crypto ')}</li>
      <li>{t<string>('Private Keys are stored in SecuX Hardware Secure Element. Beware of phishing attacks, SecuX will never ask for your 24 word recovery seed phrase. Never share them.')}</li>
      </ul> */}
    </Banner>
  );
}
